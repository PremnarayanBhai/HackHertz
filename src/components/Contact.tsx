...
export const Contact: React.FC = () => {
  ...
  return (
    <SectionWrapper id="contact" stageTag="STAGE 10" accent="pink" bgVariant="cyber" animVariant="lift" className="py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <ScrollReveal variant="fade-up">
          <div className="text-center space-y-4 max-w-3xl mx-auto">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-yellow-400/10 border border-yellow-400/30 text-yellow-400 font-pixel text-xs">
              <span>STAGE 10: GET IN TOUCH</span>
            </div>
            <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight">
              CONTACT <span className="font-pixel text-yellow-400 neon-text-yellow">ORGANIZERS</span>
            </h2>
            <p className="text-slate-300 text-lg">
              Have questions about travel logistics, domain tracks, or custom sponsorship? We are here to help.
            </p>
          </div>
        </ScrollReveal>
        ...
        <div className="space-y-4">
          <ScrollReveal variant="fade-up">
            <h3 className="font-pixel text-xs text-slate-400 uppercase tracking-widest text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {organizers.map((org, idx) => (
              <ScrollReveal key={org.id} variant="pixel-pop" delay={idx * 0.08}>
                <div
                  onMouseEnter={() => soundManager.playHover()}
                  className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-yellow-400/50 transition-all text-center space-y-3 h-full"
                >
                  <div className="w-20 h-20 rounded-full overflow-hidden mx-auto border-2 border-yellow-400/80 shadow-[0_0_15px_rgba(250,204,21,0.3)]">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-mono text-yellow-400 block">{org.role}</span>
                  </div>

                  <div className="pt-2 text-xs text-slate-400 space-y-1 font-mono">
                    <p className="line-clamp-1">{org.email}</p>
